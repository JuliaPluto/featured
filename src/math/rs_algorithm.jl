...
Try plugging in your own name:
"""

# ╔═╡ ef2fac57-ff72-400a-bc18-2e4eadafac11
begin
	s = collect("PLUTOROCKS");
	Text(String(s))
end
...
function schensted_insert!(rows, k, i=1)
	...
	for (j, m) in pairs(row)
		k < m || continue
		row[j] = k
		return schensted_insert!(rows, m, i+1)
	end
	push!(row, k)
	return rows, i
end
...
function rs_pair(j)
	...
	for (n, k) in enumerate(j)
		...
		# record index for j in Q in the same row i
...
md"""
What happens if we plug in a permutation of the numbers from ``1`` through ``N`` instead?
"""
...
# ╔═╡ f9536da7-8743-43d2-a9b8-b432ead8c99d
md"""
Recreation of [https://math.mit.edu/events/stanley70/savethedate/]():
"""
...
# ╔═╡ 0f6bc3dd-505b-4000-80db-fa17d39ee47a
function _schensted_insert!(rows, k, i=1; log=nothing)
	if i > length(rows)
		log!(log, rows, (i, 1), k)
	...
	for (j, m) in pairs(row)
		...
		log!(log, rows, (i, j), k)
		row[j] = k
		return _schensted_insert!(rows, m, i+1; log)
	end
...
begin
	...
	X = [j for i in 1:nrows(Z) for j in 1:length(row(Z, i))]
...
function small_yt(yt)
	...
	$(HTML(repr("text/html", yt; context=:html_class => class_yt)))
	"""
end
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "4f520b01b3ee163a7b5bac5e8d4929b787dae93f"

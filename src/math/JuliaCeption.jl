...
begin
	...
	function Base.show(io::IO, m::MIME"text/html", input::TwoDimInput)	
		...
		if(input.show_grid)
			...
			# get major tick spacing from width and height
			xmajor_ticks_spacing = grid_width/num_ticks
			ymajor_ticks_spacing = grid_height/num_ticks
			...
			grid_CSS = Dict(
				...
				:background => "
					  repeating-linear-gradient(
						transparent, 
						transparent $(xmajor_ticks_spacing-1)px, 
						#000 $(xmajor_ticks_spacing)px),
					  repeating-linear-gradient(
						90deg, 
						transparent, 
						transparent $(ymajor_ticks_spacing-1)px, 
						#000 $(ymajor_ticks_spacing)px);
				",
				:width => "$(grid_width)px",
